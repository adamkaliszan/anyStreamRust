...
pub fn uniform_gen_min_max(intensity: f64, variance: f64) -> (f64, f64) {
    let e_val = 1f64/intensity;

    let delta_t = f64::sqrt(12f64 * variance);
...
pub fn gamma_get_scale_shape(intensity: f64, variance: f64) -> (f64, f64) {
    let shape = variance *intensity;
    let scale = 1f64/intensity / shape;

    (scale, shape)
}

pub fn pareto_get_scale_shape(intensity: f64, variance: f64) -> (f64, f64) {
    ...
    let tmp = f64::sqrt(1f64 + par_e * par_e / variance);
    let alpha = 1f64 + tmp;
    ...
    assert_approx_eq!(chck_d, variance, 0.001f64);
